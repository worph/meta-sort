...
export function DynamicFieldRenderer({
  ...
  const getValue = (): any => {
    ...
    if (fieldKey in metadata) {
      return (metadata as any)[fieldKey];
    }
  ...
  switch (fieldType) {
    ...
    case 'json':
      ...
      const jsonData = (() => {
        ...
        // Pattern like "stream/*" - aggregate all matching keys
        const patternPrefix = schema.pattern.replace(/\{n\}.*$/, '').replace(/\*$/, '').replace(/\/$/, '');
        const result: Record<string, any> = {};

        // First, check flat keys (e.g., "stream/0", "stream/1")
        for (const [key, val] of Object.entries(metadata)) {
          if (key.startsWith(patternPrefix + '/') || key === patternPrefix) {
            // Store with the key suffix after the prefix
            const relativePath = key.substring(patternPrefix.length + 1) || key;
            result[relativePath] = val;
          }
        }

        // Also check nested objects for backwards compatibility
        const findMatching = (obj: any, currentPath: string = '') => {
          if (!obj || typeof obj !== 'object') return;

          for (const [key, val] of Object.entries(obj)) {
            // Skip flat keys we already processed
            if (key.includes('/')) continue;

            const newPath = currentPath ? `${currentPath}/${key}` : key;
            if (newPath.startsWith(patternPrefix)) {
